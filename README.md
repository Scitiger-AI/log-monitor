...
当在同一台远程服务器上监控超过 **10 个日志文件** 时，可能会出现以下错误：
...
│   └─────────────┘               └─────────────┘            │
│         │                             │                     │
│         ├── Channel 1 (tail -f log1)  │                     │
│         ├── Channel 2 (tail -f log2)  │                     │
│         ├── Channel 3 (tail -f log3)  │                     │
│         ├── ...                       │                     │
│         └── Channel N (tail -f logN)  │  ← 超过限制时失败    │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

- 当前实现：每个服务器复用 **1 个 SSH 连接**，每个日志文件占用 **1 个 Channel**
- 大多数 SSH 服务器默认配置 `MaxSessions 10`，即每个连接最多 10 个 channel

#### 解决方案

如果需要监控超过 10 个日志文件，可以修改远程服务器的 SSH 配置：
...
> ⚠️ **注意**：修改此配置需要服务器 root 权限，且会影响所有 SSH 连接。
